...
$$
f(x) = \sum_{k=1}^m p(y)f(x|y)  = \sum_{k=1}^m \phi_k f(x|k),
$$ {#eq-gmm-marg-pdf}
...
def marginal_pdf(x):  
  return sum([weight * ss.norm(**param).pdf(x) for weight, param in zip(weights, norm_params)])
...
x_samples = np.array([ss.norm(**norm_params[y]).rvs() for y in y_samples])
